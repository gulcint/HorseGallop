...
## Summary
Uygulama ilk açıldığında boş görünmesin diye içerik tabloları doldurulur: at cins listesi, at ipuçları, ahırlar, dersler, challenge'lar, app_content.

## SQL Seed Dosyası

Oluşturulacak dosya: `supabase/seed.sql`
...
**Layer 1 — Seed Dosyası**
- [ ] `supabase/seed.sql` dosyasını oluştur (yukarıdaki SQL)
- [ ] `supabase db reset --db-url <url>` veya Dashboard SQL Editor'da çalıştır

**Layer 2 — Doğrulama**
- [ ] `SELECT COUNT(*) FROM horse_breeds` → 12 kayıt
- [ ] `SELECT COUNT(*) FROM lessons` → 6 kayıt
- [ ] `SELECT COUNT(*) FROM challenges` → 4 kayıt

## Acceptance Criteria
- [ ] `horse_breeds` 12 kayıt içeriyor
- [ ] `lessons` 6 kayıt, hepsi gelecek tarihli
- [ ] `horse_tips` TR + EN 10 kayıt
- [ ] `challenges` 4 aktif challenge
- [ ] `app_content` TR + EN temel key'ler mevcut
